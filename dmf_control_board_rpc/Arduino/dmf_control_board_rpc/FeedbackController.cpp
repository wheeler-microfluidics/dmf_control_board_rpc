...
uint16_t FeedbackController::measure_impedance(uint16_t n_samples_per_window,
  ...
  FeedbackReading reading;

  reading.hv_resistor_index =
      parent_->config_settings_.n_series_resistors(HV_CHANNEL_INDEX) - 1;
  reading.fb_resistor_index =
      parent_->config_settings_.n_series_resistors(FB_CHANNEL_INDEX) - 1;

  set_series_resistor_index(HV_CHANNEL_INDEX, reading.hv_resistor_index);
  set_series_resistor_index(FB_CHANNEL_INDEX, reading.fb_resistor_index);
  ...
  float transfer_function[
    sizeof(parent_->config_settings_.A0_series_resistance)/sizeof(float)];
  for (uint8_t i=0; i < sizeof(transfer_function)/sizeof(float); i++) {
    float R = parent_->config_settings_.series_resistance(HV_CHANNEL, i);
    float C = parent_->config_settings_.series_capacitance(HV_CHANNEL, i);
    #if ___HARDWARE_MAJOR_VERSION___ == 1
  ...
  for (uint16_t i = 0; i < n_sampling_windows; i++) {
    ...
    float hv_rms, fb_rms;
    uint16_t &hv_pk_pk = reading.hv_pk_pk;
    uint16_t &fb_pk_pk = reading.fb_pk_pk;
    ...
    if (saturated_[HV_CHANNEL_INDEX] > 0) {
      reading.hv_resistor_index = -saturated_[HV_CHANNEL_INDEX];
    } else {
      reading.hv_resistor_index = series_resistor_indices_[HV_CHANNEL_INDEX];
    }
    if (saturated_[FB_CHANNEL_INDEX] > 0) {
      reading.fb_resistor_index = -saturated_[FB_CHANNEL_INDEX];
    } else {
      reading.fb_resistor_index = series_resistor_indices_[FB_CHANNEL_INDEX];
    }
    ...
    if (parent_->auto_adjust_amplifier_gain() &&
      ...
      float measured_voltage = hv_rms * transfer_function[reading.hv_resistor_index];
      ...
      if (abs(measured_voltage - target_voltage) >
          parent_->config_settings_.voltage_tolerance) {
    ...
    parent_->on_feedback_measure(reading);
